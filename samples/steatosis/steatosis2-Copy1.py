...
class SteatosisDataset(utils.Dataset):
    ...
    def load_steatosis(self, dataset_dir, subset):
        ...
        if subset!='':
            ...
            subset_dir = "stage1_test" 
        
            dataset_dir = os.path.join(dataset_dir, subset_dir)
        else:
            mode = 'pred'
        #datset_dir='/labs/konglab/xiaoyuan/MyProject_2/Mask_RCNN-xiaoyuan/datasets/steatosis/stage1_train'
        if subset == "val":
            image_ids = VAL_IMAGE_IDS
        else:
            if mode == 'pred':
                image_ids = next(os.walk(dataset_dir))[2]
                print("In steatosis2.py, waling image_ids {}".format(image_ids))
            # Get image ids from directory names
            #print('dataset_dir: {}'.format(dataset_dir))
            #print('walk: {}'.format(next(os.walk(dataset_dir))))
            #print ("**********************\n")
            else:
        ...
        for image_id in image_ids:
            ...
            #path=os.path.join(dataset_dir, "{}/{}.png".format(image_id,image_id))
            path=os.path.join(dataset_dir, "{}".format(image_id))
          
            #print("Check the path is: {}".format(path))
            self.add_image(
                "steatosis",
                image_id=image_id,
                path=os.path.join(dataset_dir, "{}".format(image_id)))
